...
namespace WPR
{
    class WPR
    {
        static void Main()
        {
            ...
            try
            {
                newAsm = AssemblyDefinition.ReadAssembly("C:\\temp\\FNWP72.dll");
            }
            catch
            {
                Debug.WriteLine("C:\\temp\\FNWP72.dll not found! App running halted.");
                return;
            }
            ...
            AssemblyNameReference reference = AssemblyNameReference.Parse
                ("WPR.MonoGameCompability");
            AssemblyNameReference referenceRuntime = AssemblyNameReference.Parse
                ("System.Runtime");
            ...
            TypeDefinition typedef = patchMono.MainModule.GetType
            (
                "WPF.MonoGameCompabilityPatch", 
                "SpriteBatchPatch"
            );
            ...
            foreach (var refer in module.AssemblyReferences) 
            {
                if 
                (   
                    refer.Name.Contains("Microsoft.Xna") 
                    && 
                    (!refer.Name.Contains("GamerServices"))
                )
                {
            ...
            //RnD
            Type tt = typeof(String); //assem.GetType("Mortar.TheGame");
